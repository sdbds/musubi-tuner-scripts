...
# model_path
$dataset_config = "./toml/task01_kv_flux2.toml"                                 # path to dataset config .toml file | 数据集配置文件路径 (task02改task02_qversion_flux2.toml)
$dit = "./ckpts/diffusion_models/flux-2-klein-base-9b.safetensors"               # DiT directory | Klein-base-9B (官方推荐训练用)
$vae = "./ckpts/vae/flux2-vae.safetensors"                                    # VAE directory | FLUX.2 Klein专用32通道VAE

# FLUX.2 Model
$model_version = "klein-base-9b"                                                # model version | 官方推荐训练用klein-base-9b (非蒸馏全血版)
$text_encoder = "./ckpts/text_encoder/qwen_3_8b.safetensors"               # Text Encoder | Klein-9B用Qwen3 8B (由安装脚本下载)
$fp8_text_encoder = $true                                                       # use fp8 for Text Encoder | 20GB显存建议开启
$fp8_scaled = $false                                                            # 禁用FP8以支持LyCORIS
...
$max_train_epochs = 12                                                           # max train epochs | 最大训练轮数（少量样本需要多轮）
$gradient_checkpointing = $true                                                    # 梯度检查，开启后可节约显存，但是速度变慢
$gradient_checkpointing_cpu_offload = $true                                       # 梯度检查cpu offload，开启后可节约显存，但是速度变慢
$gradient_accumulation_steps = 1                                                  # 梯度累加数量，变相放大batchsize的倍数
$guidance_scale = 4.0                                                             # guidance scale | klein-base-9b 推荐 4.0
...
# Learning rate | 学习率
$lr = "1"                                                                       # learning rate | Prodigy 设为 1 开启自动搜索
...
$vae_dtype = "bfloat16"                                                         # fp16 | fp32 | bf16, default: float32 for FLUX.2
$fp8_base = $False                                                              # fp8
$max_data_loader_n_workers = 0                                                  # max data loader n workers | 最大数据加载线程数
$persistent_data_loader_workers = $False                                        # persistent data loader workers | DataLoader 常驻 worker

$blocks_to_swap = 8                                                            # 交换的块数
$use_pinned_memory_for_block_swap = $False                                       # use pinned memory for block swap | 块交换使用 pinned memory
...
$output_name = "flux2_klein9b_task01_kv"                                         # output model name | 模型保存名称 (task02改task02_qversion)
$save_every_n_epochs = "1"                                                      # save every n epochs | 每1轮保存一次
...
#sample | 输出采样图片
$enable_sample = $true                                                          # 开启出图
$sample_at_first = 0                                                            # 是否在训练开始时就出图
$sample_prompts = "./toml/task01_kv_flux2_sample.txt"                             # prompt文件路径 (task02改task02_qversion_flux2_sample.txt)
$sample_every_n_epochs = 1                                                      # 每1个epoch出一次图
