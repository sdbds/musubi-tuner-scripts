"""
Musubi Tuner GUI — Shared Theme System
Palette synchronized with qinglong-captions GUI.

Single CSS Custom Properties system — no duplicate themes, no !important abuse.
Light/dark mode switching via CSS variables on html/body dark-mode and light-mode classes.
"""
...
def _input_styles() -> str:
    """Inputs, selects, toggles, sliders."""
    return """
/* ===== Inputs ===== */
.ql-input .q-field__control,
.ql-select .q-field__control,
...
.ql-input .q-field__control:hover,
.ql-select .q-field__control:hover,
.modern-input .q-field__control:hover,
.modern-select .q-field__control:hover {
    border-color: var(--ql-border-hover);
}

.ql-input.q-field--focused .q-field__control,
.ql-select.q-field--focused .q-field__control,
...
.slider-edit-input .q-field__control {
    min-height: 30px;
    height: 30px;
}

.slider-edit-input .q-field__native,
.slider-edit-input input {
    font-size: 12px;
    padding: 2px 8px;
    outline: none;
    background: var(--ql-input-bg);
...
def _component_styles() -> str:
    """Misc reusable component styles."""
    return """
/* ===== Path selector ===== */
...
/* ===== Settings / dynamic config rows ===== */
.config-row-actions .q-btn,
.inline-action-btn {
    min-height: 28px;
}
"""
...
/* Icons — no global default color; rely on local styles/inheritance */
...
html.light-mode body { background: #f7f0e6; color: #392f2b;
       --q-primary: #80618a !important; --q-color-primary: #80618a !important; }
html.dark-mode body { --q-primary: #7ee2a5 !important; --q-color-primary: #7ee2a5 !important; }
...
def apply_theme(theme_name: str = "modern", use_green_gold: bool = False):
    ...
    """
    ui.add_head_html(_THEME_BOOTSTRAP, shared=True)
