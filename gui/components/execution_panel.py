...
from components.log_viewer import create_log_viewer, LogViewer
from utils.job_manager import JobStatus, job_manager
from utils.process_runner import ProcessResult, ProcessStatus
...
class ExecutionPanel:
    ...
    使用方式:
        panel = ExecutionPanel(start_label=t("start_cache"), height="50vh")

        # 在 Start 按钮回调里:
        await panel.run_job(
            script_key="musubi_tuner.cache_latents",
            args=[...],
            name="Cache Latents",
    ...
    对于 show_start=False 模式，各工具的 Start 按钮手动调用
    ...
    async def run_job(
        ...
        *,
        ...
        Args:
            script_key: Python 模块路径，如 'musubi_tuner.cache_latents'
            args: 脚本参数列表
            name: Job 显示名
            runner_kwargs: 透传给 ProcessRunner 的额外参数（可含 use_accelerate=True）
        ...
        try:
            ...
            result = ProcessResult(ProcessStatus.ERROR, -1, t("task_stopped", "任务已停止"))
            try:
                self.log_viewer.info(result.message)
            except RuntimeError:
                pass
            finally:
                self._safe_set_running(False)
        ...
        try:
            if job.status == JobStatus.CANCELLED:
                ...
                self.log_viewer.success(t("task_finished", "任务完成"))
                if on_success:
                    on_success(result)
            else:
                self.log_viewer.error(t("task_failed", "任务失败"))
                if on_failure:
                    on_failure(result)
        except RuntimeError:
            # 页面已离开，元素已销毁
    ...
    def cancel(self):
        ...
        try:
            self.log_viewer.info(t("task_stopped", "任务已停止"))
            ui.notify(t("task_stopped", "任务已停止"), type="info")
        except RuntimeError:
            pass
