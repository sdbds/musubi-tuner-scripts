...
class SetupStep:
        ...
        with ui.column().classes(get_classes('page_container') + ' gap-4'):
            ...
            with ui.card().classes(get_classes('card') + ' w-full q-pa-md'):
                ...
                with ui.row().classes('w-full items-center justify-between q-pa-sm q-mt-sm').style(f'''
                    background: var(--ql-secondary-muted);
                    border-radius: 10px;
                    border: 1px solid var(--ql-secondary-border);
                ''') as torch_row:
                    with ui.row().classes('items-center gap-2'):
                        ui.icon('local_fire_department', size='20px')
    ...
    def _run_checks(self):
        ...
        if py_version >= (3, 8):
            ...
            self.python_status.text = f'❌ {py_version.major}.{py_version.minor} {t("python_requires_38")}'
