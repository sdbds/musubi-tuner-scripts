...
# Cache mode(HunyuanVideo、Wan、FramePack、flux_kontext、qwen_image、longcat)
$cache_mode = "longcat_Lora" # Cache mode | 缓存模式
...
$vae_tile_min_size = ""                                                    # Minimum spatial size for LongCat VAE tiling
...
if ($cache_mode -ieq "Wan") {
  ...
  elseif ($cache_mode -ieq "longcat") {
    $script_path = "longcat_"
    [void]$ext2_args.Add("--text_encoder=$t5")
    if ($fp8_t5) {
      [void]$ext2_args.Add("--fp8_t5")
    }
    if ($longcat_i2v) {
    ...
    if ($vae_tile_min_size) {
      [void]$ext_args.Add("--vae_tile_min_size=$vae_tile_min_size")
    }
  }
}
...
python -m accelerate.commands.launch "./musubi-tuner/$($script_path)cache_latents.py" `
  ...
  --vae=$vae $ext_args
Assert-NativeCommandSucceeded "Command failed: 2.7long_cat_cache_latent_and_text_encoder.ps1"
