...
$generate_mode = "longcat"

# Parameters from hv_generate_video.py
$dit = "./ckpts/diffusion_models/longcat_dit.safetensors"   # DiT directory | DiT路径
...
$guidance_scale = 4.0 # guidance scale for classifier free guidance, wan is 3.0 for 480P,5.0 for 720P  (default 5.0)
...
# LongCat specific parameters
$longcat_i2v = $false
$disable_numpy_memmap = $false
$longcat_flow_target = "x1_minus_x0"
...
$video_size = "960 1024" # video size
...
elseif ($fp8 -and $generate_mode -ine "longcat") {
...
if ($flow_shift -ne 3.0 -and $generate_mode -ine "longcat") {
...
if ($generate_mode -ieq "Wan") {
    ...
    if ($image_path -and ($generate_mode -ine "flux_kontext" -and $generate_mode -ine "qwen_image" -and $generate_mode -ine "longcat")) {
        [void]$ext_args.Add("--image_path=$image_path")
    }
    if ($end_image_path -and ($generate_mode -ine "flux_kontext" -and $generate_mode -ine "qwen_image" -and $generate_mode -ine "longcat")) {
        [void]$ext_args.Add("--end_image_path=$end_image_path")
    }
    if ($control_image_path -and ($generate_mode -ine "longcat")) {
        [void]$ext_args.Add("--control_image_path=$control_image_path")
    }
    else {
        if ($generate_mode -ine "longcat") {
    ...
    elseif ($generate_mode -ieq "longcat") {
        $script = "longcat_generate_video.py"
        if ($t5) {
            [void]$ext_args.Add("--text_encoder=$t5")
        }
        if ($fp8_t5) {
            [void]$ext_args.Add("--fp8_t5")
        }
        if ($longcat_i2v) {
            [void]$ext_args.Add("--longcat_i2v")
        }
        if ($vae_cache_cpu) {
            [void]$ext_args.Add("--vae_cache_cpu")
        }
        ...
        if ($fp8_fast -and $fp8_scaled) {
            [void]$ext_args.Add("--fp8_fast")
        }
        if ($longcat_flow_target) {
            [void]$ext_args.Add("--flow_target=$longcat_flow_target")
        }
        if ($guidance_scale) {
            [void]$ext_args.Add("--guidance_scale=$guidance_scale")
        }
    ...
    if ($generate_mode -ine "qwen_image" -and $generate_mode -ine "longcat") {
        [void]$ext_args.Add("--text_encoder1=$text_encoder1")
        [void]$ext_args.Add("--text_encoder2=$text_encoder2")
    }
    if ($embedded_cfg_scale -ne 10.0 -and $generate_mode -ine "longcat") {
...
python -m accelerate.commands.launch "./musubi-tuner/$script" --dit=$dit `
    ...
    --save_path=$save_path `
    $ext_args
Assert-NativeCommandSucceeded "Command failed: 5.7long_cat_generate.ps1"
