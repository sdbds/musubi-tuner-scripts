...
# Cache mode(HunyuanVideo、Wan、FramePack)
$cache_mode = "FramePack" # Cache mode | 缓存模式
...
$vae = "./ckpts/framepack/hunyuan_video_vae_fp32.safetensors"
...
python -m accelerate.commands.launch "./musubi-tuner/$($script_path)cache_latents.py" `
  ...
  --vae=$vae $ext_args
Assert-NativeCommandSucceeded "Command failed: 2.3framepack_cache_latent_and_text_encoder.ps1"
