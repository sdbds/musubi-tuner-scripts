...
$dit = "./ckpts/hunyuan-video-i2v-720p/transformers/mp_rank_00_model_states.pt"     # DiT directory | DiT路径
$vae = "./ckpts/hunyuan-video-i2v-720p/vae/pytorch_model.pt"                        # VAE directory | VAE路径
...
$output_name = "wan_qinglong"  # output model name | 模型保存名称
...
python -m accelerate.commands.launch --num_cpu_threads_per_process=8 $launch_args "./musubi-tuner/$laungh_script.py" `
  ...
  --logging_dir="./logs" `
  $ext_args
Assert-NativeCommandSucceeded "Command failed: 3.1hy_train_lora.ps1"
