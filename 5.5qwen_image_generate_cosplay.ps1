...
#Generate Mode (HunyuanVideo/Wan/FramePack/flux_kontext/qwen_image)
...
$text_encoder1 = "./ckpts/text_encoder/t5xxl_fp16.safetensors" # Text Encoder 1 directory
...
$fp8_fast = $false # Enable fast FP8 arthimetic(RTX 4XXX+)
...
$image_path = "train\image\nudefilter\filter\13.jpg" # image path
...
# LoRA
$lora_weight = "./output_dir/qwen_image_cosplay-step00020000.safetensors" # LoRA weight path
$lora_multiplier = "1.0" # LoRA multiplier
$lora_weight_high_noise = "" # LoRA weight path for high noise
...
$prompt = """<2b> from (nier:automata) is depicted in a vibrant and festive cosplay outfit, embodying a seductive and cheerful holiday spirit. She wears a red Santa-inspired leotard with a high-leg cut, showcasing her thighs and accentuating her figure. The leotard features a prominent heart-shaped cutout in the center, revealing her cleavage and adding a provocative element to the design. The outfit is adorned with fluffy white fur trim along the edges, giving it a cozy yet alluring appearance. A green bow is tied at the center of the cutout, complementing the holiday theme. She wears elbow-length red gloves with matching fur trim, which she playfully holds to her chest, emphasizing her pose. Her outfit is completed with pink thigh-high stockings, secured with garter straps, and a pair of fake reindeer antlers attached to a red headband, adding a whimsical touch. Her white hair, styled in a neat bob cut with a slight sweep over one eye, is complemented by the festive red hairband. She stands indoors, with a soft, warm-toned background that is slightly blurred to keep the focus on her. Her brown eyes gaze directly at the viewer, and her lips curve into a cheerful smile, with a hint of blush on her cheeks adding to her charming expression. The lighting is soft and even, highlighting the details of her costume and creating a gentle, inviting atmosphere. The overall composition is a cowboy shot, capturing her from the thighs up, with her hands resting on her chest and her body leaning slightly forward, creating a dynamic and engaging pose.
"""
$from_file = "" # Read prompts from a file
$video_size = "1216 864" # video size
$video_length = 1 # video length
$fps = 30          # Framepack default is 30
$infer_steps = 25 # number of inference steps
$save_path = "./output_dir" # path to save generated video
$seed = 1026 # Seed for evaluation.
$sample_solver = "dpm++" # sample solver ["unipc", "dpm++", "vanilla"]
...
$fp8_scaled = $false # use fp8 scaled for DiT model
...
$output_type = "images" # output type
...
if ($generate_mode -ieq "Wan") {
    ...
    if ($image_path -and ($generate_mode -ine "flux_kontext" -and $generate_mode -ine "qwen_image")) {
        [void]$ext_args.Add("--image_path=$image_path")
    }
    if ($end_image_path -and ($generate_mode -ine "flux_kontext" -and $generate_mode -ine "qwen_image")) {
        [void]$ext_args.Add("--end_image_path=$end_image_path")
    }
    ...
    elseif ($generate_mode -ieq "qwen_image") {
...
python -m accelerate.commands.launch "./musubi-tuner/$script" --dit=$dit `
    ...
    --save_path=$save_path `
    $ext_args
Assert-NativeCommandSucceeded "Command failed: 5.5qwen_image_generate_cosplay.ps1"
