...
#训练模式(HunyuanVideo_Lora、HunyuanVideo_db、Wan_Lora、FramePack_Lora、flux_kontext_Lora、qwen_image_Lora、longcat_Lora)
$train_mode = "longcat_Lora"
...
# LongCat
$longcat_i2v = $False                                                               # Enable Image-to-Video conditioning
$longcat_flow_target = "x1_minus_x0"                                                # Flow matching supervision target: x1_minus_x0 | x0_minus_x1
...
$fp8_fast = $True                                                                  # Enable scaled FP8 matmul kernels (requires SM 8.9+)
...
$disable_numpy_memmap = $False                                                      # Disable numpy memmap when loading weights (universal option)
...
if ($train_mode -ilike "HunyuanVideo*" -or $train_mode -ilike "FramePack*" -or $train_mode -ilike "flux_kontext*" -or $train_mode -ilike "qwen_image*" -or $train_mode -ilike "longcat*") {
  ...
  elseif ($train_mode -ilike "longcat*") {
    $laungh_script = "longcat_" + $laungh_script
    $network_module = "networks.lora_longcat"
    ...
    if ($longcat_flow_target -ieq "x1_minus_x0") {
  ...
  if ($train_mode -inotlike "qwen_image*" -and $train_mode -inotlike "longcat*") {
...
if ($fp8_scaled) {
  ...
  if ($train_mode -ilike "Wan*" -or $train_mode -ilike "longcat*" -and $fp8_fast) {
...
python -m accelerate.commands.launch --num_cpu_threads_per_process=8 $launch_args "./musubi-tuner/$laungh_script.py" `
  ...
  --logging_dir="./logs" `
  $ext_args
Assert-NativeCommandSucceeded "Command failed: 3.6long_cat_train_lora.ps1"
