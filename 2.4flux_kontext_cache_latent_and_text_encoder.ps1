...
$cache_mode = "flux_kontext" # Cache mode | 缓存模式

# Cache lantent
$dataset_config = "./toml/qinglong-flux-kontext-datasets.toml"            # path to dataset config .toml file | 数据集配置文件路径
$vae = "./ckpts/vae/ae.safetensors"
...
python -m accelerate.commands.launch "./musubi-tuner/$($script_path)cache_latents.py" `
  ...
  --vae=$vae $ext_args
Assert-NativeCommandSucceeded "Command failed: 2.4flux_kontext_cache_latent_and_text_encoder.ps1"
